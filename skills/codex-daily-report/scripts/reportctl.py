...
def build_parser() -> argparse.ArgumentParser:
    ...
    show.add_argument("--type", required=True, choices=("weekly",))
...
def command_collect(args: argparse.Namespace) -> int:
    ...
    output = atomic_write_json(args.output, bundle)
    print(json.dumps({
        "status": "ok",
...
def command_show(args: argparse.Namespace) -> int:
    ...
    state_path = month_dir / f"codex-run-state-weekly-{scope}.json"
    if not report_path.exists() or not state_path.exists():
        return validation_failure([{
            "code": "validated_report_missing",
            "message": "Validated weekly report or run-state is missing.",
        }])
    ...
    if (
        state.get("version") != 1
        or state.get("report_type") != "weekly"
        ...
        return validation_failure([{
            ...
            "message": "Weekly report does not match its validated run-state.",
        }])
